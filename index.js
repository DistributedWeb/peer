...
module.exports = DWebPeer

var debug = require('debug')('@dwebs/peer')
...
var RTCDWebPeerConnection = typeof window !== 'undefined'
  ? window.mozRTCPeerConnection || window.RTCPeerConnection ||
    window.webkitRTCPeerConnection
  : wrtc.RTCDWebPeerConnection
...
inherits(DWebPeer, stream.Duplex)

/**
 * WebRTC peer connection. Same API as node core `net.Socket`. Duplex stream.
 * @param {Object} opts
 */
function DWebPeer (opts) {
  var self = this
  if (!(self instanceof DWebPeer)) return new DWebPeer(opts)
  if (!opts) opts = {}
  ...
  extend(self, {
    initiator: false,
    stream: false,
    config: DWebPeer.config,
    constraints: DWebPeer.constraints,
    channelName: (opts && opts.initiator) ? hat(160) : null,
    trickle: true
  }, opts)

  if (typeof RTCDWebPeerConnection !== 'function') {
  ...
  self._debug('New dWeb Peer (initiator: %s)', self.initiator)
  ...
  self._pc = new RTCDWebPeerConnection(self.config, self.constraints)
  self._pc.oniceconnectionstatechange = self._onIceConnectionStateChange.bind(self)
  self._pc.onbeamingstatechange = self._onBeamingStateChange.bind(self)
...
/**
 * Expose config and constraints for overriding all DWebPeer instances. Otherwise, just
 * set opts.config and opts.constraints when constructing a DWebPeer.
 */
DWebPeer.config = {
  iceServers: [
    {
...
DWebPeer.constraints = {}

DWebPeer.prototype.send = function (chunk, cb) {
  var self = this
  if (!cb) cb = noop
  self._write(chunk, undefined, cb)
}

DWebPeer.prototype.beam = function (data) {
  var self = this
  if (self.destroyed) throw new Error('Cannot Beam After dWeb Peer Has Been Destroyed!')
  ...
  if (!data.sdp && !data.candidate) {
    self._destroy(new Error('beam() called with invalid beam data'))
  }
}

DWebPeer.prototype.destroy = function (onclose) {
  var self = this
  self._destroy(null, onclose)
}

DWebPeer.prototype._destroy = function (err, onclose) {
  var self = this
  ...
  if (self._pc) {
    ...
    self._pc.onbeamingstatechange = null
...
DWebPeer.prototype._setupData = function (event) {
  var self = this
...
DWebPeer.prototype._setupVideo = function (stream) {
  var self = this
  self._pc.addStream(stream)
}

DWebPeer.prototype._read = function () {}
 ...
 * @param {function} cb
 */
DWebPeer.prototype._write = function (chunk, encoding, cb) {
  var self = this
  if (self.destroyed) return cb(new Error('Cannot Write After dWeb Peer Has Been Destroyed!'))
...
DWebPeer.prototype._createOffer = function () {
  var self = this
  if (self.destroyed) return
  ...
  self._pc.createOffer(function (offer) {
    ...
    var sendOffer = function () {
      self.emit('beam', self._pc.localDescription || offer)
    }
...
DWebPeer.prototype._createAnswer = function () {
  var self = this
  if (self.destroyed) return
  ...
  self._pc.createAnswer(function (answer) {
    ...
    var sendAnswer = function () {
      self.emit('beam', self._pc.localDescription || answer)
    }
...
DWebPeer.prototype._onIceConnectionStateChange = function () {
  var self = this
  if (self.destroyed) return
...
DWebPeer.prototype._maybeReady = function () {
  var self = this
...
DWebPeer.prototype._onBeamingStateChange = function () {
  var self = this
  if (self.destroyed) return
  self.emit('beamingStateChange', self._pc.beamingState)
  self._debug('beamingStateChange %s', self._pc.beamingState)
}

DWebPeer.prototype._onIceCandidate = function (event) {
  var self = this
  if (self.destroyed) return
  if (event.candidate && self.trickle) {
    self.emit('beam', { candidate: event.candidate })
...
DWebPeer.prototype._onChannelMessage = function (event) {
  var self = this
  if (self.destroyed) return
  var data = event.data
...
DWebPeer.prototype._onChannelOpen = function () {
  var self = this
...
DWebPeer.prototype._onChannelClose = function () {
  var self = this
  if (self.destroyed) return
...
DWebPeer.prototype._onAddStream = function (event) {
  var self = this
  if (self.destroyed) return
...
DWebPeer.prototype._onError = function (err) {
  var self = this
  if (self.destroyed) return
...
DWebPeer.prototype._debug = function () {
  var self = this
